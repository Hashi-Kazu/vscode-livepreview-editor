...
- ■■□ R-28-15 ドラッグ選択範囲が左右余白にはみ出さず、かつ長文の上部・中盤・末尾やスクロール後でも確実に視認できること（`.cm-selectionLayer` に `width:100%` を与えたうえで `clip-path: inset(0 40px 0 48px)` で左右余白を除外する）。選択レイヤーの高さは固定しないこと。`height: 100%` で viewport 高に固定すると、スクロール文書座標で配置される選択矩形がその高さで縦方向にクリップされ、文書途中以降のハイライトが消えるため。
