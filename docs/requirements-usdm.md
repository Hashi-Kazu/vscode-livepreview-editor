...
> ▶️ **開発継続中（2026-06-26 時点 / v1.21.0）**: v1.11.0 の開発凍結は v1.12.0 で解除済み。v1.21.0 では、ビューア操作中に別 Markdown ソースへ切り替えた場合だけ、追従更新後に対象ビューアへフォーカスを戻す挙動を追加した（R-03）。改めて凍結する場合は本バナーを凍結表記に戻し、凍結理由（品質安定・スコープ確定）を踏まえて判断すること。
...
> **説明：** `livePreview.openWith` は標準ソースエディタを置換せず、編集可能な `WebviewPanel` を横に開く。`customEditors` と `livePreview.toggleSource` は提供しない。同一 URI は常に 1 ビューアとし、異なる URI は複数ビューアを許可する。`livePreview.followActiveEditor` が有効な場合、アクティブな Markdown ソースへ最後に操作したビューアが追従する。ビューア操作中に別 Markdown ソースへ切り替えた場合だけ、切り替え完了後に対象ビューアへフォーカスを戻す。切り替えは保留中の Webview 編集の適用後に行い、世代番号で旧バインド由来の遅延メッセージを拒否する。
...
- ■■□ R-03-05 `livePreview.followActiveEditor` は既定値 `true` とし、有効時はアクティブな Markdown ソースエディタへ最後に操作したビューアを追従させること。対象 URI を既に別ビューアが表示している場合は既存ビューアを所有者として維持し、重複切り替えを行わないこと。無効時、非 Markdown、または同一 Markdown URI への切り替えでは自動切り替えおよびフォーカス復元状態を破棄すること。
...
- ■■□ R-03-09 書式コマンドとアクティブエディタ追従の対象は最後に操作したビューアとし、ビューア操作後にソースへフォーカスを戻しても対象を保持すること。ビューア操作中に別 Markdown ソースへ切り替えた場合だけ、`onDidChangeViewState` で記録した直前アクティブビューア状態を `onDidChangeActiveTextEditor` の追従処理で一回限り消費し、文書切り替え完了後に対象 `WebviewPanel.reveal(..., false)` でビューアへフォーカスを戻すこと。標準エディタ操作中の Markdown 切り替えではフォーカスを奪わないこと。対象 URI を既に表示しているビューアがあれば、そのビューアへ復帰すること。
