...
> ▶️ **開発継続中（2026-06-23 時点 / v1.13.0）**: v1.11.0 の開発凍結は v1.12.0 で解除済み。v1.13.0 では、アコーディオンを開いても本文が表示されない不具合を修正（`detectDetailsBlocks` に本文行を追加し `details-block` ウィジェットで描画、R-27-06 追加）、ブロックウィジェット（表・アコーディオン）の高さ会計のズレによるクリック位置と編集位置のずれを修正（`estimatedHeight` 実装・開閉時の再測定、R-28-10 追加）、テーブルのセル内テキストをカーソルがブロック内にあるとき編集可能化（非カーソル時のみウィジェット、R-22-02 再定義 / R-22-01 改訂）を行う。改めて凍結する場合は本バナーを凍結表記に戻し、凍結理由（品質安定・スコープ確定）を踏まえて判断すること。
...
- ■■□ R-28-10 ブロックウィジェット（`TableWidget`・`DetailsWidget`）は `block: true` で挿入されるため、`get estimatedHeight()` を実装して実描画に近い高さ（px）を返し、CodeMirror のブロック高さ会計と実 DOM のズレを抑えること（高さズレはウィジェットより**下の行**の `posAtCoords` をずらし、クリック位置と編集位置の不一致を招く）。目安は 1 行≈22px とし、テーブル＝（ヘッダ 1＋本文行数）×行高＋余白、アコーディオン＝閉時約 1 行・開時（1＋本文行数）×行高。`DetailsWidget` は開閉で高さが変わるため、`toDOM(view)` で受け取った `view` を使い `toggle` 時に `view.requestMeasure()` を呼んで再測定させること。
