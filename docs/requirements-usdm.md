...
- ■■□ R-03-11 ソースタブを閉じて Live Preview のみで編集しているとき、`applyEdit`（`workspace.applyEdit`）または `performSave`（`document.save()`）呼び出しの副作用として VS Code 側が対象 URI のソースエディタタブを自動的に再表示した場合は、当該タブを直ちに閉じること（`livePreview.suppressSourceAutoOpen` 設定、既定値 `true`）。判定は各呼び出し直前・直後の可視タブ URI 集合の差分（`decideAutoOpenedTabsToClose`）で行い、呼び出し前から可視だったタブ（ユーザーが自発的に開いた可能性があるもの）は対象外として閉じない。v1.29.0 以降、主対策は apply 直後の即時保存による dirty 窓最小化（R-03-08）へ移行したが、本抑制処理はコア側挙動に対する backstop として残置する（撤去しない）。
