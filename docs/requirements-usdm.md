...
> ▶️ **開発継続中（2026-08-06 時点 / v1.51.6）**: v1.51.6 で、GitHub Issue #92（表セル・アコーディオンで `snake_case_word` のような語中アンダースコアを含むテキストが、`case` 部分だけ誤って斜体表示されてしまう不具合）に対応した。本文エディタの `computeDecorations`（`src/core/model.ts`）は R-01-06 の CommonMark 準拠語境界規則（`underscoreBoundaryOk`：マッチ直前・直後が ASCII 英数字でないこと）をアンダースコア強調 (`_..._`/`__..__`) に適用していたが、表セル・アコーディオンの共有描画経路 `appendInlineCell`（`src/webview/decorations.ts`）はこの判定を呼んでおらず、単純な `_([^_]+)_`/`__([^_]+)__` 正規表現マッチだけで斜体・太字化していた。`underscoreBoundaryOk` を `model.ts` からexportし、`appendInlineCell` のトークナイズループでアンダースコア系マッチ（`__..__`/`_..._`）にのみ同一ロジックの境界判定を追加し、境界NGの場合はそのマッチを強調タグ化せず地の文として扱うようにした（正規表現走査は自然に前進するため無限ループは発生しない）。`<br>` トークナイズ・`code`/`**`太字/`*`斜体の既存判定・優先順位・正規表現パターン自体、および `model.ts` 側の `underscoreBoundaryOk` のロジック・既存呼び出し箇所は変更していない（R-01-06 改訂）。
> ▶️ **開発継続中（2026-08-03 時点 / v1.51.5）**: v1.51.5 で、GitHub Issue #90（表セルの `<input>` 編集中、Shift+Enter で `<br>` を挿入した直後や以降のキャレット移動・入力時に、キャレットの表示位置が実際のキャレット位置とずれる不具合）に対応した。Issue #86（`6a62598`）で新設した `syncCellInputCaretScroll`（`src/webview/main.ts`）が `<input>` の可視コンテンツ幅を算出する際、`getComputedStyle(input).paddingRight` のみを `clientWidth` から差し引き、`paddingLeft`（`.cm-lp-table-cell-input` は `padding: 6px 13px`、`box-sizing: border-box`）を差し引いていなかった。この過大な `clientWidth` が `computeCaretScrollLeft`（`src/core/editing.ts`、ロジック・シグネチャ不変）に渡され、キャレットが実際にはまだ入力欄の右端を超えて隠れているにもかかわらず可視と誤判定し、スクロール量が `paddingLeft` 分（13px）不足していた。`clientWidth` の算出を `input.clientWidth - paddingLeft - paddingRight` に修正した。`computeCaretScrollLeft` 自体・`insertBrAtCaret`／`classifyTableWidgetPress`／`.cm-lp-table-cell-input { height: 100% }`（Issue #85）・commit/cancel ロジック・イベント隔離・`appendInlineCell` の `<br>` トークナイズはいずれも変更していない（R-22-10 改訂）。
> ▶️ **開発継続中（2026-08-03 時点 / v1.51.4）**: v1.51.4 で、GitHub Issue #86（表セルの `<input>` 編集中に Shift+Enter で `<br>` を挿入した後、続けてテキストを入力してセル幅からはみ出しても、キャレット追従スクロール（`input.scrollLeft`）が更新されず入力中の内容が見えなくなる不具合。Issue本文の「Ctrl+Enter」は誤記で、実装・R-22-10 の要件通り `Shift+Enter` が正しいトリガー）に対応した。`src/core/editing.ts` に純粋関数 `computeCaretScrollLeft(caretOffset, clientWidth, currentScrollLeft)` を新設し、キャレットのピクセル位置が可視範囲 `[scrollLeft, scrollLeft + clientWidth]` の外にあれば、その範囲に収まる最小限の `scrollLeft` を返すようにした。`src/webview/main.ts` の `startCellEdit` は `<input>` の `input` イベント（`<br>` 挿入直後の合成イベントも含む）ごとに、キャンバス（`measureTextWidthPx`、`<input>` の計算済みフォントを使用）でキャレット直前までのテキスト幅を測り、`computeCaretScrollLeft` の結果を `input.scrollLeft` へ適用する（`syncCellInputCaretScroll`）。従来（Issue #83、`d14f87b`）は `<br>` 挿入直後に合成 `input` イベントを1回 dispatch してキャレットの再描画（ペイント）のみを保証しており、可視領域へのスクロール追従までは保証していなかった。`blur`/`focus` の強制リセットは用いておらず、既存の commit/cancel ロジック（Enter/Escape/Tab/blur、IME `isComposing` 判定）・イベント隔離対象（mousedown/click/dblclick/input/contextmenu/copy/cut/paste の `stopPropagation`）・`insertBrAtCaret`／`updateTableCell`（`src/core/tableEdit.ts`）の純粋関数シグネチャ・`classifyTableWidgetPress`（Issue #82）・`.cm-lp-table-cell-input { height: 100% }`（Issue #85）はいずれも変更していない（R-22-10 改訂）。
> ▶️ **開発継続中（2026-08-03 時点 / v1.51.3）**: v1.51.3 で、GitHub Issue #85（同じ行の他セルが `<br>`（R-22-10）や折り返しで2行以上になり編集対象セルより行が高くなっている表で、編集中セル内の余白部分をクリックすると常にキャレットが文末へ戻ってしまう）に対応した。`media/editor.css` の `.cm-lp-table-cell-input` に `height: 100%;` を追加し、`<input>` が編集中セル（`<td>`/`<th>`、`padding: 0`）の描画領域全体を確実に覆うようにした。従来は `height` 指定がなく、行が他セルの内容でより高くなると `<input>` は上端に張り付き、その下の余白は素の `<td>`/`<th>` になっていたため、余白クリック時の `event.target` が `.cm-lp-table-cell-input` に一致せず（`insideCellInput` が `false`）、`classifyTableWidgetPress`（Issue #82）が `'begin-cell-edit'` に分類してセル編集を再開始し、`startCellEdit` の意図的な末尾キャレット配置（Issue #79）が発動していた。CSS のみの修正であり、`classifyTableWidgetPress`・`startCellEdit` のキャレット末尾配置ロジック・ネイティブキャレット配置分岐（Issue #82）・`<br>` 挿入ロジック（Issue #83）はいずれも変更していない（R-22-08 改訂）。
...
>
> ▶️ **開発継続中（2026-07-20 時点 / v1.42.0）**: v1.42.0 で、GitHub Issue #53（Tab キーでリストのインデント幅がマーカー幅に沿わない）に対応した。(1) Tab はキャレット行を「直前の非空行」の子要素にし、その直前項目の本文開始位置（先頭インデント＋マーカー文字数＋マーカー直後の空白）をそのまま新しいインデント幅として採用するよう `changeListIndent`（`src/core/editing.ts` 新設）を実装した。箇条書き `-`/`*`/`+` は 2 スペース、`1. `/`10. ` のような番号付きリストは桁数に応じて 3/4 スペースというように、リストの種類・番号桁数にかかわらず子リストの開始位置が親項目の本文位置に揃う。直前項目が既にそのキャレット行の親であるなど、これ以上インデントできない場合は変化させない（Tab キー自体は消費し既定 Tab へフォールバックしない）。(2) Shift+Tab はキャレット行より浅いインデントを持つ直前の項目を遡って探し、その項目自身のインデント幅へ戻すことで階層を 1 段階戻す。最上位項目では変化しない。(3) Webview の `indentCommand`（`src/webview/main.ts`）を単一行専用から選択範囲全行対応へ拡張し、複数行が選択されている場合は編集前のドキュメントのスナップショットを基準に各行のインデント幅を独立に算出したうえで、CodeMirror の 1 トランザクションとしてまとめて適用する。これにより同じ階層の兄弟項目をまとめて Tab／Shift-Tab しても、既に編集された行を参照して意図せずさらに深く／浅くネストする（カスケード）ことなく、選択行全体が同じ新しい階層に揃う。リストではない行は既存動作を維持する：Tab はリスト行を含まない選択では従来どおり既定キーマップへフォールバックし、Shift-Tab は従来どおり `changeIndent` による固定幅（先頭スペース最大 2、またはタブ 1 つ）の除去を行う。空行はリストの継続とみなして直前の非空行を参照し、コードブロック等の非リスト行はマーカーとして解釈されず変更されない。インデントには常にスペースを使用し、タブ文字は挿入しない。純粋ロジックは `test/feature.editing.test.ts` の `changeListIndent` 単体テストおよび `indentCommand` と同一ロジックを再現した `EditorState` 経由のエンドツーエンド回帰テストで検証した。ユーザーの Markdown 本文は引き続き書き換えない（R-01-02）。既存の `continueList`／`computeListEnterEdit`（Enter によるリスト継続、R-23）は変更していない（R-24-01/02 改訂、R-24-03/04/05 新設）。
>
> ▶️ **開発継続中（2026-07-21 時点 / v1.41.0）**: v1.41.0 で、GitHub Issue #51（入れ子引用とリストの本文位置・行高、深い番号付きリストの Enter）に対応した。(1) 引用 l1〜l6 のセレクタを `.cm-line.cm-lp-quote-lN` とし、CodeMirror 基底テーマの `.cm-content > .cm-line` 以上の詳細度で、階層ごとの `padding-left: 16px`〜`96px` を実 Webview に適用する（バー位置・幅 3px・色は不変、R-02-05）。(2) 箇条書きと番号付きリストを共通の固定幅 `.cm-lp-list-marker` 外枠と内側 glyph/number 要素で描画し、同一 indent の本文開始位置と通常行相当の行高を統一した。0 階層目の番号付きリストも非カーソル行では元のアラビア数字＋区切りをウィジェット化し、カーソル行では生記法を維持する（R-01-05、R-01-07、R-28-06）。(3) リスト継続 Enter のキーマップだけを `Prec.highest` で登録し、4 階層目でも既定・言語キーマップより先に `handleEnter` へ到達させる。`continueList`／`computeListEnterEdit` の計算ロジックと番号加算・indent 維持・空項目終了は変更しない（R-23-02、R-23-04、R-23-05）。ユーザーの Markdown 本文は引き続き書き換えない（R-01-02）。
>
> ▶️ **開発継続中（2026-07-20 時点 / v1.40.0）**: v1.40.0 で、GitHub Issue #41（リスト階層別スタイルと入れ子引用表示の調整）に対応した。(1) 箇条書きの基本アイコンサイズ（`.cm-lp-list-bullet` の `font-size`）を `1.2em` から `1.4em` へ拡大し、2 段目の ○（`.cm-lp-list-bullet-hollow` の `0.55em`）以外の •/▪ を大きくした。拡大したグリフが `.cm-line` の unitless `line-height: 1.6` を継承して行ボックス（1.6×1.4em）を基本行ボックス（1.6em）より高くし、•/▪ 行だけリストアイテム間余白が広がっていた不具合を、bullet の `line-height` を `1`（固定）へ変更して解消し、•/▪ 行と ○ 行のアイテム間余白を全階層で揃えた（R-01-05）。(2) 入れ子引用の 1 段あたりのインデント段差を `2em`（Issue #36）から `16px` へ改め、`l1` がバー（x=0）とテキスト（padding-left 16px）の間に確保する間隔と段差を一致させた。これにより各入れ子階層のバーが親階層のテキスト列にちょうど整列し、`2em` 段差時にバーが親テキスト列を追い越して入れ子表示が崩れていた不具合を解消した（`l1` の単独引用は不変、バー本数・幅 3px・色は不変、R-02-05）。(3) 番号付きリスト 4 階層目（インデント 6）末尾 Enter の継続不具合は、純粋部 `computeListEnterEdit`（`src/core/editing.ts`）および Webview キーマップ経路がインデント 6/8 のいずれでも正しい継続を返すことを検証済みで、実 Webview 依存の再現が必要なため純粋部は変更せず、markdown() パーサを載せた `EditorState` 経由のエンドツーエンド回帰テスト（4・5 階層目）を追加した（挙動は不変、R-23）。ユーザーの Markdown 本文は引き続き書き換えない（R-01-02）。色はすべて `var(--vscode-*)` テーマ変数追従を維持する（R-28-04）。
>
...
>
> ▶️ **開発継続中（2026-07-20 時点 / v1.37.9）**: v1.37.9 で、GitHub Issue #31（4 階層目以降のリスト継続・視認性の指摘）を調査した結果、純粋ロジック（`continueList`／`ULIST_LINE`／`OLIST_LINE`／`bulletLevel`・`numeralLevel` クランプ）自体は正しく、真因は回帰テスト未整備と、2 段目以降のグリフ統一（Issue #21）により深い階層ほど視覚的な階層差が分かりにくくなっていたこと、および仕様書の `font-size` 記述齟齬であると判明した。(1) `continueList` の 4 階層目以降（indent 6/8）の箇条書き・番号付きリスト・タスクの継続を `test/feature.editing.test.ts` に回帰テストとして追加し、`test/feature.issue16.decorations.test.ts` に 3 段目・4 段目が同一グリフでも `indent` 属性は異なる（階層差自体は保持される）ことを検証するテストを追加した。(2) 深い階層でも段差がより明確に見えるよう、リスト・タスクの行装飾インデント（`Decoration.line` の `padding-left`）を `Math.floor(indent/2)` 段あたり `1.5em` から `2em` へ拡大した（`indent` 属性の算出・生ソース・`continueList` は不変、R-01-02）。(3) R-01-05 本文中の `.cm-lp-list-bullet` の `font-size` 記述が実装（`1.6em`、v1.37.7 で変更済み）と齟齬していた `1.4em` の誤記を修正した。純粋ロジックは変更していない。ユーザーの Markdown 本文は引き続き書き換えない（R-01-02）。
>
...
- ■■■ R-01-06 アンダースコア強調（`_`・`__`）は ASCII 単語文字に挟まれた語中（例 `my_var_name`）では発火しないこと（CommonMark 準拠。アスタリスクは語中可）。本規則は `src/core/model.ts` の `computeDecorations` 本文判定だけでなく、表セル・アコーディオン（`<details>`）の `appendInlineCell`（`src/webview/decorations.ts`）にも同様に適用すること（Issue #92）。
...
- ■■□ R-01-05 リスト `-` / `1.` を検知し、`-` マーカーをビュレットウィジェットへ置換（非カーソル行）。ビュレットは階層（インデント 2 スペースを 1 段として `Math.floor(indent/2)`）に応じて `•`（0 段目）/`○`（1 段目）/`▪`（2 段目以降はすべて▪と同様）のいずれかに切り替え、色はリンク色ではなく本文文字色（`var(--vscode-editor-foreground)`）に追従すること。○（1 段目）グリフは字形の見え方の都合上 •/▪ と同じ `font-size` では他階層より大きく見えるため、他階層と視覚サイズを揃えるよう僅かに小さく描画すること（Issue #24）。基本アイコンサイズ（`.cm-lp-list-bullet` の `font-size`）は `1.4em` とし（2 段目の ○ 以外のすべての階層がこの `1.4em` の箱で表示される）、○（2 段目、`.cm-lp-list-bullet-hollow`）は `0.55em` とする（Issue #41 で `1.2em`→`1.4em`、○ は不変）。•/▪ の拡大したグリフが `.cm-line` の unitless `line-height: 1.6` を継承すると行ボックス（`1.6 × 1.4em`）が基本行ボックス（`1.6em`）を超えて •/▪ 行のリストアイテム間余白だけが広がるため、`.cm-lp-list-bullet` の `line-height` を `1`（固定）とし、•/▪ 行と ○ 行のアイテム間余白を全階層で揃えること（Issue #41）。2 段目以降のグリフは統一されるため、階層差は `R-28-06` の行装飾インデント（`padding-left`）で表現し、生成される Markdown 本文・indent 文字列（`R-01-02`）は変更しない。
  Issue #51 以降、箇条書きと番号付きリストの置換マーカーは、固定幅・固定高さ・固定 `line-height`・共通右余白を持つ `.cm-lp-list-marker` 外枠と内側 glyph/number 要素に分離し、同一 indent の本文開始位置と通常テキスト相当の行高を両形式で統一すること。内側の •/▪ は `1.4em`、○ は `0.55em` の見た目を維持し、外枠・内側ともリスト固有の上下 `margin`／`padding` を追加しないこと。
- ■■□ R-01-07 番号付きリスト `1.`/`1)` は 0 段目（インデント 0〜1）も、非カーソル行では元のアラビア数字と区切りを共通 `.cm-lp-list-marker` 外枠のウィジェットへ置換し、1 段目はローマ数字小文字（`i.`/`ii.`/…）、2 段目以降はすべてアルファベット小文字（`a.`/`b.`/…）へ置換すること。カーソル行では全階層とも変換せず生記法（アラビア数字）を表示すること（R-01-01）。
...
- ■■□ R-02-05 引用 `> quote` を装飾し、`>` マーカーを非カーソル行で隠す。入れ子引用（`>>`・`>>>` … 最大 6 段）は `>` の連続数を階層深度として検知し、行クラスに `cm-lp-quote-l{1〜6}`（7 段以降は `l6` に丸める）を付与して段数に応じた左インデント・境界線幅を CSS で表現すること（純粋関数は深度算出のみを担い、色は引き続き `var(--vscode-*)` テーマ変数に追従、R-28-04）。マーカー非表示範囲は従来通り全 `>` とその直後の空白を覆うこと。入れ子引用は各階層の独立ボーダーバーを祖先階層分まとめて描画し、n 段目の行には n 本のボーダーが表示されること（ボーダー本数・位置は不変のまま、テキスト開始位置は Issue #26 で全階層 `padding-left` 末尾定数を `12px` から `16px` へ広げ、さらに Issue #36 で 1 段あたりのインデント段差を `1.4em` から `2em` へ拡大したが、Issue #41 で 1 段あたりの段差を `16px`（＝`l1` がバー x=0 とテキスト padding-left 16px の間に確保する間隔と同一）へ改め、各入れ子階層のバーが親階層のテキスト列にちょうど整列するようにした（`l2` は `padding-left: 32px`／バー `0, 16px`、`lN` は `padding-left: N×16px`／バー `0, 16px, …, (N-1)×16px`）。これにより `2em` 段差時にバーが親テキスト列を追い越して入れ子表示が崩れていた不具合を解消した（`l1` の単独引用は `padding-left: 16px`／バー1本で不変、バー本数・幅 3px・色は不変）。空行を挟まない非空の継続行（`>` の再掲がない、または再掲があっても直前の引用行より浅い深度の行）は CommonMark の lazy continuation に倣い、直前の引用行の深度を継承して同じ `cm-lp-quote-l{1〜6}` の行クラス・インデント・背景バンドを維持すること。継続行に `>` マーカーが無い場合はマーカー hide を追加しない。空行、または見出し・水平線・フェンスコード・表・アコーディオン・数式ブロック・GitHub Alerts など引用以外のブロック開始で継続はリセットされ、以降の非引用行は通常の段落として描画されること。
  Issue #51 以降、l1〜l6 の CSS セレクタは CodeMirror 基底テーマ `.cm-content > .cm-line`（詳細度 0,2,0）以上の詳細度を持たせ、実 Webview でも `padding-left: 16px, 32px, …, 96px` を確実に適用すること。
...
- ■■□ R-02-08 GitHub Alerts（`> [!NOTE]` / `> [!TIP]` / `> [!IMPORTANT]` / `> [!WARNING]` / `> [!CAUTION]` の 5 種）を純粋関数 `detectAlertBlocks(lines, code)` で検知すること。開始行は `[!TYPE]` マーカーのみを内容に持つ引用行で、以降の連続する引用行を本文とする。フェンスコードブロック内の `> [!NOTE]` は素通し（リテラル文字列として扱い alert としない）。非カーソル行では、各行に行クラス `cm-lp-alert cm-lp-alert-{kind}`（開始行 `cm-lp-alert-open`／終了行 `cm-lp-alert-close`）を付与し、`>` マーカーを hide、開始行の `[!TYPE]` ラベルは種別アイコン＋タイトル名を描く `alert-title` replaceWidget へ置換して生ラベルを可視テキストとして残さないこと。表示は「左端線中心」ではなく背景色を持つ自然なカードとし、全行に per-kind の淡い背景（`color-mix`）を敷いたうえで、開始行（`cm-lp-alert-open`）で上側左右、終了行（`cm-lp-alert-close`）で下側左右の角を丸め、左右両端が閉じた 1 枚のカードに見せること。終了行には下端・下角・背景が途中で切れないよう十分な下パディングを与える。種別ごとのアクセント色（アイコン・タイトル・左ボーダー・背景）は `--vscode-*` テーマ変数に追従（フォールバックのみ、R-28-04）。種別とアイコン・色は次のグルーピングとする（Issue #36）: Note＝青系＋ペン（pencil）、Tip・Important＝シアン系＋炎（flame）、Warning・Caution＝オレンジ系＋警告三角形（alert-triangle）。折りたたみ表示（`<details>` アコーディオン、R-27）ともカードデザイン（背景・角丸・パディング）を統一する。カーソル行では生記法（`> [!NOTE]`）を維持しウィジェットを出さない（R-01-01）。装飾は表示のみで入力文字列を変更しない（R-01-02）。行装飾ベースのため block widget を用いず、ブロック高さ会計（R-28-10/11）に影響しない（R-02-08 新設・#7）。
...
- ■■□ R-22-10 セル編集の `<input>` で Shift+Enter を押すと `commit` を行わず、カーソル位置へ `<br>` を挿入してカーソルをその直後へ置くこと。挿入は常に `<br>`（小文字・スペースなし・自己終了なし）の表記で統一すること。表示側（`appendInlineCell`）は `<br>`・`<br/>`・`<br />`（大文字小文字を問わない）を実際の改行（`<br>` 要素）として描画すること。カーソルが表ブロック内にあるだけで `<input>` を開いていない生テキスト編集モードは対象外とし、既存の行分割・box-drawing 判定に影響しないこと。`<br>` 挿入直後および以降の通常入力を通じて、キャレットが常に `<input>` の可視領域内に収まるよう `input.scrollLeft` を追従させること（キャレット位置の可視性、Issue #86）。可視幅の算出は `<input>` の左右パディング（`.cm-lp-table-cell-input` の `padding: 6px 13px`）を両方差し引いた `clientWidth` を用いること。片側（`paddingRight`）のみを差し引くとキャレットが実際には隠れているのに可視と誤判定し、右端超過時のスクロール量が不足すること（Issue #90）。
...
- ■■□ R-23-02 順序リストは番号をインクリメントすること。リスト継続 Enter のバインドは `Prec.highest` で単独登録し、CodeMirror の既定・Markdown 言語キーマップより先に `handleEnter` へ到達すること。
- ■■■ R-23-03 タスクは未完了チェックボックス `[ ]` で継続すること。
- ■■□ R-23-04 インデントを維持すること。4 階層目（indent 6）の `      1. d` 末尾 Enter でも同階層の `      2. ` を生成し、キャレットを新項目末尾へ置くこと。
...
> **説明：** 純粋関数 `changeListIndent`（`src/core/editing.ts`）でリスト行のインデント幅を算出し、Webview の Tab/Shift-Tab キーマップ（`indentCommand`、`src/webview/main.ts`）が適用する。リストではない行は `changeIndent` による従来の固定幅動作を維持する。

###### ＜リスト行：マーカー幅に沿ったインデント＞

- ■■□ R-24-01 リスト行で Tab を押すと、直前の非空行（同一リストの前の項目）のマーカー＋マーカー直後の空白の幅（本文開始位置－先頭インデント）を 1 段幅として、対象行自身の現在インデントへ加算すること。箇条書き `-`/`*`/`+` は 1 回につき 2 スペース、番号付きリスト `1. ` は 3 スペース、`10. ` のように桁数が増えれば 4 スペースというように、リストの種類・番号の桁数に応じた 1 段だけを現在位置から深くすること。`1) ` のような閉じ括弧区切りでも同様に算出し、直前項目が対象行より深い場合や対象行が既にインデントされている場合も、直前項目の絶対階層をコピーせず常に 1 段だけ加算すること。直前の非空行が存在しない、またはリスト行でない場合は変化させないこと。インデントには常にスペースを使用し、タブ文字を挿入しないこと。
- ■■■ R-24-02 リスト行で Shift+Tab を押すと、キャレット行より前方かつインデント幅が狭い直前の項目を遡って探し、その項目自身のインデント幅（本文開始位置ではなく先頭インデント幅）へ戻すことで階層を 1 段階戻すこと。該当する項目が無い場合はインデント 0（最上位）へ戻し、既に最上位の場合は変化させないこと。
- ■■■ R-24-03 複数行が選択されている場合、選択範囲内の対象リスト行をまとめてインデント／アウトデントすること。各行のインデント幅は編集前のドキュメントのスナップショットを基準に独立して算出し、選択範囲内で既に変更された行を参照して意図せずさらに深く／浅くネストしないこと（同じ階層の兄弟項目を選択して Tab/Shift-Tab した場合、全行が同じ新しい階層に揃うこと）。複数階層にわたって Tab／Shift-Tab を繰り返してもインデントが崩れないこと。

###### ＜リストではない行：既存動作の維持＞

- ■■■ R-24-04 Tab を押した行（または選択範囲）にリスト行が 1 つも含まれない場合、既定のキーマップへフォールバックすること（従来の Tab 動作を維持する）。
- ■■■ R-24-05 Shift+Tab はリストではない行に対しても、先頭スペース（最大 2）またはタブ 1 つを除去する従来の固定幅アウトデントを維持すること。空行・コードブロックなどの非リスト行はマーカーとして解釈されず、この固定幅アウトデント以外では変更されないこと。
...
- ■■□ R-27-04 サマリ行は背景バー・枠・余計な padding を持たず、`▶`（閉）/`▼`（開）の三角マーカー＋サマリテキストのみの軽量な 1 行表示とし、本文インライン要素と馴染ませること。マーカーは開閉状態に追従する（`<details open>` で `▼`、閉で `▶`）。サマリテキストは通常ウェイト（`font-weight: 400`、太字継承を防ぐ）で表示し、マーカーとサマリテキストの間に半角程度の余白（マーカーの `margin-right` 約 `0.4em`）を設けること（MAIO 参照画像準拠）。ただし `<details>` 要素全体（`.cm-lp-details`）は Issue #36 で GitHub Alerts と共通のカードデザイン（淡い `color-mix` 背景・角丸・上下パディング）を持ち、折りたたみ時は上下左右すべて角丸の完結したカード、展開時はサマリ部と本文部が同一背景で継ぎ目なくつながり下端が切れないようにすること（サマリ行自体は依然として独自の背景バー・枠を持たない）。
...
- ■■□ R-28-05 本文体裁を GitHub / VS Code 標準 Markdown プレビュー（github-markdown-css 風）に寄せること。具体的には次を満たすこと（描画エンジンと装飾ロジックは変更せず CSS の体裁のみで実現する）:
  ...
  - 引用 `.cm-lp-quote`・表 `table.cm-lp-table`（ボーダー・ヘッダ背景・任意のゼブラ。ヘッダ/データ行の差別化の詳細は R-28-18 に委ねる）・水平線 `.cm-lp-hr-line` を GitHub プレビュー風にする。水平線の上下余白は `padding: 0.05em 0` 目安まで詰め、区切り線単体の視覚的な間延びを抑えること（R-28-14 に従い margin ではなく padding で表現）。
...
- ■■□ R-28-18 表ヘッダ行（`table.cm-lp-table th`）とデータ行（`td`）を明確に差別化すること。ヘッダはデータ行のゼブラより一段強い背景（`color-mix` によるテーマ追従色。非対応エンジン向けフォールバック宣言を先置き）、`font-weight: 700`、下端罫線（`border-bottom`、`var(--vscode-panel-border)`、太さは 1px 維持で高さ会計 R-28-11 に影響させない）で強調する。色はすべて `var(--vscode-*)`／`color-mix` でテーマ追従（R-28-04）し、ライト/ダーク両テーマでコントラストを確保する。DOM 構造・装飾ロジックは変更せず CSS のみで実現し、ユーザー本文は変更しない（R-01-02）。
...
- ■■□ R-35-05 ドキュメントの変更（Live Preview でのローカル編集の self-echo、または標準ソースエディタでの直接編集などの external 変更のいずれも含む）に起因する標準ソースエディタの可視範囲変化（`onDidChangeTextEditorVisibleRanges`、リフローやキャレット追従によるもの）は、ユーザーによる明示スクロールとして Webview へ中継しないこと。`onDocumentChanged`（`src/livePreviewCustomEditorProvider.ts`）は対象 URI 一致判定の直後・self-echo/external 分岐の前で毎回、host 側編集スクロール抑止窓（`editScrollSuppressUntil`）を `nextScrollSuppressUntil`（`EDIT_SCROLL_SUPPRESS_WINDOW_MS`、`src/core/viewport.ts`）で開始すること。`onSourceVisibleRangesChanged` は既存の `scrollSuppressUntil` チェックに加え、`isEchoScroll` でこの窓が開いている間 `scrollTo` メッセージの送信を抑止すること。既存の `scrollSuppressUntil`／`localEditScrollSuppressUntil`／`lastSyncedScrollLine` デデュープ・`onWebviewScroll` の `revealRange`、および R-35-01/02/03/04 の同期方向・ループ防止ロジックは変更しないこと。標準ソースエディタと Live Preview を並べて開いた状態で、どちら側で改行操作を行っても双方の縦スクロール位置が意図せず先頭へジャンプしないこと（手動確認）。
