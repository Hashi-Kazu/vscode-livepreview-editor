...
> ⏸️ **凍結保留／開発継続中（2026-06-22 時点 / v1.7.0）**: 当初 v1.7.0（ビュー切り替えの同一タブ再描画化〔保存確認ダイアログを出さない〕・`<details>` アコーディオン折りたたみ〔既定で折りたたみ。見出しガター折りたたみは廃止〕・左余白の拡大・文字色のテーマ追従）で開発凍結を予定していたが、**凍結の方針は一旦保留**とし、もうしばらく開発を継続する。以降も通常どおり機能追加・変更・バグ修正を行う。改めて凍結する場合は本バナーを凍結表記に戻し、凍結理由（品質安定・スコープ確定）を踏まえて判断すること。
...
- ■■□ R-03-05 切り替えは結果として同一タブ内で再描画されたように見せること。Live↔標準は別エディタ入力のため `vscode.openWith` が新タブを生成するので、目標ビューを `viewColumn: Active` で開いたうえで、`window.tabGroups` を走査して同一 `uri` の旧ビュー種別のタブを特定し、それがまだ残っており新しいアクティブタブと別物であれば `window.tabGroups.close` で閉じること。新エディタは同一 `TextDocument` を参照し続けるため、未保存変更があっても旧タブを閉じる際に保存確認ダイアログは出ないこと。
...
> **説明：** `detectTableBlocks`/`parseTable` でブロックを解析。非カーソル時は単一の `table-block` ウィジェットへ置換して HTML テーブル描画。表内にカーソルがあるときは生の行を表示して編集可能にする。

###### ＜描画＞

- ■■■ R-22-01 非カーソル時、表ブロック全体を 1 つの `table-block` ウィジェットに置換し、ヘッダ・整列・行データを保持すること。
- ■■■ R-22-02 表内にカーソルがあるときは生の行（`table-row`）を表示すること。コードブロック内の表もどきは表にしないこと。
...
> **説明：** HTML の `<details><summary>…</summary>…</details>` ブロックをプレビュー上でアコーディオンとしてレンダリングする。判定は純粋関数 `detectDetailsBlocks`（`src/core/model.ts`）が担い、非カーソル時はブロック全体を 1 つの `details-block` ウィジェット（既定で折りたたんだ＝閉じた状態）へ置換する。Webview 層（`src/webview/decorations.ts`）が実際の `<details>` 要素にマッピングし、`<summary>` クリックで開閉する。ブロック内にカーソルがあるときは生の HTML を表示して編集可能にする。見出し（`#`）単位のガター折りたたみ（旧 `foldService`/`foldGutter`）は v1.7.0 で廃止した。

###### ＜折りたたみ＞

- ■■■ R-27-01 `<details><summary>…</summary>…</details>` ブロックを検知し、非カーソル時はブロック全体を 1 つの `details-block` ウィジェットへ置換すること。フェンスコードブロック内の `<details>` はアコーディオンとみなさず、閉じタグの無い未終了ブロックは折りたたまない（文末まで畳まない）こと。
- ■■■ R-27-02 アコーディオンは既定で折りたたんだ（閉じた）状態で表示し、`<summary>` をクリックで開閉できること。
- ■■■ R-27-03 ブロック内にカーソルがあるときは生の HTML 記法を表示し、編集可能であること（置換しない）。
...
- ■■■ R-27-05 アクティブ（カーソルが内側にある）な `<details>` ブロックでも、構造 HTML タグ（`<details …>`・`<summary …>`・`</summary>`・`</details>` の山括弧を含むタグ文字列）はカーソル有無に関わらず常に隠し、生のタグ文字列がユーザーに見えないこと。判定は純粋関数 `detailsTagRanges`（`src/core/model.ts`）が各行のタグ範囲を `hide` 記述子として返し、Webview が `Decoration.replace` にマップする。`<summary>` と `</summary>` の間のサマリ本文は可視・編集可能な生テキストとして残すこと。装飾は表示のみで入力文字列（ソース）を変更しないこと。
...
- ■■□ R-28-07 左右の読みやすい余白を「Markdown All in One」プレビューに寄せて拡大すること（`.cm-content` のパディングを左 `48px`／右 `40px` 目安に拡大）。見出しは行装飾のためインデントを増やさず、見出しと本文の左端が揃うこと。本文フォントは Markdown サンセリフスタックを明示指定（継承のみに頼らない）し、`font-weight: 400`（通常ウェイト）で等幅へフォールバックしないこと。
...
- ■■■ R-28-09 アクティブ（カーソルが内側にある）な `<details>` ブロックでも本文行のインライン記法（太字・斜体・インラインコード）を描画し、生のマーカー（例 `**ワークパッケージ**`）を表示しないこと。構造 HTML タグ（`<details …>`/`<summary …>`/`</summary>`/`</details>`）の山括弧タグ部分は R-27-05 に従い常に隠す（生のタグ文字列を表示しない）。タグ以外の本文・サマリ本文はインライン解析する。カーソル行ではインラインマーカーを隠さず生記法を保つ（Live-Preview 原則）。装飾は表示のみで入力文字列を変更しないこと。`<summary>` ウィジェットのサマリも最小限のインライン記法（`appendInlineCell`）で描画すること。
