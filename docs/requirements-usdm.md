...
> ▶️ **開発継続中（2026-06-23 時点 / v1.16.1）**: v1.11.0 の開発凍結は v1.12.0 で解除済み。v1.15.0 では、ブロックウィジェット直下のクリック位置ずれを根本修正（`toDOM` の `requestMeasure` 撤去・`estimatedHeight` をフォントサイズ／開閉状態依存へ動的化・テーブルセル `line-height` 固定、R-28-10 再定義／R-28-11 新規）、上下矢印がテーブル／アコーディオンをまたぐ際の複数行ジャンプを修正（`ArrowUp`/`ArrowDown` カスタムコマンドで 1 ソース行ずつ着地、R-28-12 新規）の 2 件を修正。v1.16.0 では、スクロール後のクリック位置ずれを根本修正（`#editor { overflow: hidden }` で二重スクロールコンテナを解消し `.cm-scroller` を唯一のスクロール所有者に一本化、R-28-13 新規）。v1.16.1 では、見出し・HR 行の `margin` → `padding` 変換によりスクロール量依存クリック位置ずれの残存原因を修正（R-28-10 でテーブルに対して行った同原則の修正が見出し・HR に適用されていなかった、R-28-14 新規）、`applyFontSize` 後の `requestMeasure` 追加、HR 線の太さと余白調整。改めて凍結する場合は本バナーを凍結表記に戻し、凍結理由（品質安定・スコープ確定）を踏まえて判断すること。
...
- ■■■ R-02-03 リンク `[text](url)` をラベル装飾＋href 保持で表示し、構文を非カーソル行で隠す。`.md` リンクのクリックは別タブ・`preview: false`（永続タブ・Live 状態）で開き、現在の Live タブの種別は変更しない（Live 維持）。`http(s)`/`mailto` は外部ブラウザ、その他相対パスは既定エディタで `preview: false`。
...
### R-03 Live／標準エディタの切り替え #toggle

> **理由：** Markdown All in One 風に、ライブ編集ビューと VS Code 標準のソースエディタを同一タブで行き来できるようにするため。プラグイン独自のソースビューは持たず、ソース表示は VS Code 標準エディタに委ねる。

> **説明：** `.md` は既定で標準テキストエディタ（ソース）で開く（`customEditor` の `priority: option`）。標準エディタのタイトルバーの目アイコン（`livePreview.openWith`）で Live エディタへ、Live エディタのタイトルバーの code アイコン（`livePreview.toggleSource`）で標準エディタへ、同一タブで切り替える。切り替えは目標ビューを `vscode.openWith`（`viewColumn: Active`）で開き、同一 `uri` の旧ビュー種別の重複タブが残れば `window.tabGroups.close` で閉じて単一タブに見せる。新エディタは同一 `TextDocument` を参照し続けるため、旧タブの close で「保存しますか？」ダイアログは出ない（ドキュメントが閉じられないため）。

###### ＜切り替え＞

- ■■□ R-03-01 標準エディタのタイトルバーボタン（`livePreview.openWith`）で、現在のタブを Live エディタへ切り替えられること。
- ■■□ R-03-02 Live エディタのタイトルバーボタン（`livePreview.toggleSource`）で、現在のタブを VS Code 標準テキストエディタ（ソース）へ戻せること。プラグイン独自のソースビュー（装飾 OFF の生表示）は持たない。
- ■■□ R-03-03 Live エディタ表示中にリンク先（`.md`）を開くと、`preview: false`（永続）・Live 状態の別タブで開き、現在の Live タブは Live のまま維持すること（外部 URL はブラウザ、その他の相対パスは `preview: false` の既定エディタ）。
- ■■□ R-03-04 いずれの切り替えでも元の Markdown テキストは変化しないこと（エディタの差し替えのみ）。
- ■■□ R-03-05 切り替えは結果として同一タブ内で再描画されたように見せること。Live↔標準は別エディタ入力のため `vscode.openWith` が新タブを生成するので、目標ビューを `viewColumn: Active` で開いた**後**に旧ビュー種別の重複タブを `findTab` で再評価し、それが現在のアクティブ（＝新規に開いた）タブと別物のときだけ `window.tabGroups.close` で閉じること（アクティブタブは決して閉じない）。新エディタは同一 `TextDocument` を参照し続けるため、未保存変更があっても旧タブを閉じる際に保存確認ダイアログは出ないこと。VS Code の API 仕様上「同一タブの無置換」は完全には保証できないため、最低限「保存確認を出さない」「重複タブを残さない」を満たすこととし、本挙動は手動 UI 検証を前提とする。`supportsMultipleEditorsPerDocument` は複数 Live エディタ共存時の同期リスクを避けるため `false` のまま維持する。
...
- ■■■ R-05-04 レンダリング失敗時はソース表示へフォールバックする（`computeDecorationsSafe`）。
...
- ■■□ R-26-02 標準リンク/オートリンクのクリックで、外部 URL はブラウザ、相対パスはファイルを開くこと。リンク先が `.md` の場合は本 Live Preview ビューア（`vscode.openWith` で viewType 指定）で開き、それ以外は既定エディタで開くこと。（UI 結線は手動確認）
