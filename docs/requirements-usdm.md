...
- ■■■ R-24-01 リスト行で Tab を押すと、キャレット行を直前の非空行（同一リストの前の項目）の子要素にし、その項目の本文開始位置（先頭インデント＋マーカー文字数＋マーカー直後の空白の合計）をそのまま新しいインデント幅とすること。箇条書き `-`/`*`/`+` は 2 スペース、番号付きリスト `1. ` は 3 スペース、`10. ` のように桁数が増えれば 4 スペースというように、リストの種類・番号の桁数に応じて子リストの開始位置が親項目の本文開始位置に揃うこと。`1) ` のような閉じ括弧区切りでも同様に算出すること。直前の非空行がリスト行でない場合、または直前項目の本文開始位置がキャレット行の現在のインデント以下（＝既にその項目の子として最大階層）の場合は変化させないこと。インデントには常にスペースを使用し、タブ文字を挿入しないこと。
