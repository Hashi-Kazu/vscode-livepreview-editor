...
> ▶️ **開発再開（2026-06-22 時点 / v1.12.0）**: v1.11.0 の開発凍結を解除して開発を再開した。v1.12.0 では表・`<details>` アコーディオンをビューア専用化（ブロック内カーソルでもウィジェットのまま＝非編集、編集は標準ソースエディタ）、アコーディオン開閉状態の保持、チェックボックス ON/OFF のホスト→Webview 反映修正、相対パスリンクの山括弧宛先除去、本文余白の調整、ビュー切り替えの保存確認回避の堅牢化を行う。改めて凍結する場合は本バナーを凍結表記に戻し、凍結理由（品質安定・スコープ確定）を踏まえて判断すること。
...
> **説明：** `detectTableBlocks`/`parseTable` でブロックを解析。表は**ビューア専用**として扱い、カーソル位置に依らず常に単一の `table-block` ウィジェットへ置換して HTML テーブル描画する（生の行を表示する編集モードは持たない）。表の編集は標準ソースエディタで行う。

###### ＜描画＞

- ■■■ R-22-01 表ブロック全体を 1 つの `table-block` ウィジェットに置換し、ヘッダ・整列・行データを保持すること。
- ■■■ R-22-02 表は**ビューア専用**とし、ブロック内にカーソルがあってもウィジェットのまま（生の行 `table-row` を表示しない＝実質非編集）とすること。Webview 側ではウィジェット本体のクリックを capture フェーズで握りつぶし（`preventDefault`/`stopImmediatePropagation`）、CodeMirror 既定の mousedown によるキャレット移動を防ぐこと。コードブロック内の表もどきは表にしないこと。
