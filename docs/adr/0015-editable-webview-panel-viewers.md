...
「ソースタブを reveal しない」という決定に反し、`workspace.applyEdit`／`document.save()` の呼び出しが VS Code コア側の副作用として対象 URI のソースタブを自動的に再表示するケースが確認された。この副作用自体は本 ADR の決定範囲外（VS Code コアの挙動）であり抑制できないため、呼び出し直前・直後の可視タブ URI 集合を比較し、新規に出現したタブのみを `vscode.window.tabGroups.close()` で閉じることで補完する（R-03-11、`livePreview.suppressSourceAutoOpen`、既定 `true`）。呼び出し前から可視だったタブ（ユーザーが自発的に開いた可能性がある）は対象外とし、誤って閉じない。なお v1.29.0（ADR-0019）以降、主対策は apply 直後の即時保存による dirty 窓の最小化へ移行し、`closeAutoOpenedSourceTab` はコア側の再表示挙動に対する backstop として残置する。

## 追記：ソースタブ自動クローズ処理の廃止（2026-07-19）

v1.29.0 でデバウンス apply 直後の即時保存を導入し、ソースタブ再表示の主原因だった TextDocument の dirty 滞留を解消した。その後も補完策として `closeAutoOpenedSourceTab` を残していたが、`workspace.applyEdit()` または `document.save()` の待機中にユーザーが自発的に開いたタブと、VS Code の副作用で表示されたタブを完全には識別できない。

拡張機能がユーザーのタブを誤って閉じる可能性を排除するため、v1.34.0 で `vscode.window.tabGroups.close()` を使用したソースタブ自動クローズ処理と `livePreview.suppressSourceAutoOpen` 設定を廃止する。以後、ソースタブ再表示への対策は、デバウンス apply＋直後保存による dirty 滞留防止に一本化する。
