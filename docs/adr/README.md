...
| [0005](0005-custom-text-editor-provider.md) | CustomTextEditorProvider の採用（復活） | v1.20.0 で ADR-0015 に置換されたが、v1.36.0 で ADR-0020 により再採用（viewType `livePreview.editor`） | 高 |
...
| [0015](0015-editable-webview-panel-viewers.md) | editable WebviewPanel ビューア（廃止） | ソース横表示、URI 単一所有、active editor follow、queued rebinding。v1.36.0 で ADR-0020 により置換 | 高 |
| [0017](0017-codemirror-history-and-ack-sync.md) | CodeMirror history and acknowledgement sync（一部 supersede） | host ack、WorkspaceEdit self-echo ledger は維持。「CodeMirror 単独 Undo」は v1.36.0 で ADR-0020 が supersede（Undo/Redo を VS Code へ委譲） | 採用 |
