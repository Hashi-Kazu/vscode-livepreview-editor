...
> ▶️ **開発継続中（2026-06-26 / v1.21.0）**: ビューア操作中の Markdown 切り替え時だけ対象ビューアへフォーカス復帰する受け入れ定義を追加した。
...
| `test/viewer.lifecycle.test.ts` | R-03-02/05/06/09（URI 重複防止・最後に操作したビューアへの追従・ビューア操作後の一回限りフォーカス復帰・バインド世代判定） |
...
- **v1.6.0 で追加・変更した以下は VS Code 上の手動確認項目（純粋ロジック外）**。VSIX インストール後に確認する:
  - R-03-01〜09（ソース横への editable WebviewPanel 起動、異なる URI の複数ビューア、同一 URI の重複防止、`.md` リンク、`livePreview.followActiveEditor` の有効／無効、最後に操作したビューアの追従、ビューア操作中に別 Markdown へ切り替えた場合だけのフォーカス復帰、保留編集後の安全な切り替え、タイトル・画像 resource base・変更リスナーの再バインド、ソースタブを閉じた後の編集継続、書式・チェックボックス・Undo・IME・CRLF・最小差分同期の維持）
