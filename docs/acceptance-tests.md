...
- **v1.6.0 で追加・変更した以下は VS Code 上の手動確認項目（純粋ロジック外）**。VSIX インストール後に確認する:
  ...
  - R-28-15（十分にスクロールできる長文を用意し、文書の上部・中盤・末尾でそれぞれドラッグ選択ハイライトが表示されること。スクロール後も選択開始位置・終了位置を変えてハイライトが消えないこと。表および `<details>` の直前・直後をまたぐ選択でも表示されること。いずれの位置でもハイライトが本文の左 48px・右 40px の余白へ漏れないこと）
