...
Explorer で Ctrl+C したファイル/フォルダーのリンク挿入（R-29-06）は Webview 経路では扱えない。Webview は OS のファイルクリップボード（`text/uri-list` を持たない）を読めないため、明示コマンド `livePreview.pasteFileAsMarkdownLink` がホスト側で Windows PowerShell の外部プロセス（`src/clipboard/readClipboardFiles.ts`、`powershell.exe -STA -EncodedCommand`）を起動し、`Clipboard.GetFileDropList()` を UTF-8 の compact JSON として読む。これは本拡張で唯一の外部プロセス依存であり、Windows 限定・host 専用ファイルへ隔離する（JSON 解釈と全リンク生成ロジックは `src/core/pasteLink.ts` の純粋関数に置き、Webview バンドルへ `child_process` を混入させない）。WebviewPanel は CustomTextEditor ではなく `activeTextEditor`/`TextEditor.edit()` を持たないため、挿入は D&D/paste と同じ `insertMedia` 経路（`requestClipboardLinkInsertion`→`clipboardLinkInsertionContext`→`insertMedia`）を再利用し、開始時 selection を `pendingMediaRequests` で追従する。
...
| Windows OS file clipboard reader (host-only) | `src/clipboard/readClipboardFiles.ts` |
| Media/link pure helpers (image, snippet, label) | `src/core/pasteLink.ts` |
