...
- **ビュー切り替え（R-03）**: 独自の「装飾 ON/OFF 生表示」ソースビューは廃止。`.md` は既定で VS Code 標準テキストエディタ（`customEditor` の `priority: option`）で開く。標準エディタ→Live は `livePreview.openWith`（タイトルバーの目アイコン）、Live→標準は `livePreview.toggleSource`。いずれも `LivePreviewEditorProvider.switchEditor` が `vscode.openWith`（`viewColumn: Active`）で対象ビューを開く。Live↔標準は別エディタ入力のため `openWith` が新タブを生成し得るので、**openWith の後**に旧ビュー種別の重複タブを `findTab` で再評価し、現在のアクティブ（新規）タブと別物のときだけ `window.tabGroups.close` で閉じて同一タブに見せる（アクティブ／dirty な入力は閉じないため「保存しますか？」ダイアログは出ない。新エディタは同一 `TextDocument` を参照し続ける）。`supportsMultipleEditorsPerDocument` は複数 Live エディタ共存時の同期リスクを避けるため `false` を維持。VS Code の API 制約上「同一タブ無置換」は完全保証できないため本挙動は手動 UI 検証前提。`activeDocument` を保持してコマンドのルーティング先を決める。リンク先 `.md` は `openLink` が viewType 指定の `vscode.openWith` で Live 状態の別タブを開く。
- **折りたたみ（R-27、ビューア専用）**: HTML の `<details><summary>…</summary>…</details>` アコーディオンを対象とする。判定は純粋関数 `detectDetailsBlocks`（`src/core/model.ts`）で、**カーソル位置に依らず常に**ブロック全体を 1 つの `details-block` ウィジェットへ置換し、Webview 側（`DetailsWidget`）が実際の `<details>` 要素（既定で閉）にマッピングする（v1.12.0 でビューア専用化。生 HTML を表示する編集モードは廃止し、本文の編集は標準ソースエディタで行う）。開閉状態は summary テキストをキーに `openDetails` 集合で記憶し再描画後も保持する。表（R-22）も同様にビューア専用で、常に `table-block` ウィジェットへ置換する。見出し単位のガター折りたたみ（旧 `@codemirror/language` の `foldService`/`codeFolding`/`foldGutter`）は廃止した。折りたたみ・表示は TextDocument を変更しない。
...
- **選択レイヤー寸法同期（R-28-15）**: `drawSelection()` が生成する `.cm-selectionLayer` は `contain:size` と absolute 子要素の組み合わせにより自身の高さを内容から決定できない。`src/webview/main.ts` の専用 `ViewPlugin` が `requestMeasure` の read フェーズで `EditorView.contentHeight` と対象レイヤーを取得し、write フェーズで inline height を文書高へ同期する。`docViewUpdate`、`geometryChanged` / `docChanged`、フォント変更、表・`<details>` の再測定に追従し、同一キーで重複要求を集約する。CSS は横幅と左右余白の `clip-path` のみを担当し、固定 height は持たない。
