...
Mermaid ダイアグラム（```mermaid、R-36）は純粋関数 `detectMermaidBlocks`（`src/core/model.ts`）が行範囲＋生コードを検知し、`computeDecorations` が既定でキャレット位置に依らずブロック全体を `mermaid-block` block replaceWidget へ置換する（ビューア専用、KaTeX/表のようなキャレット自動フォールスルーは行わない）。`src/webview/decorations.ts` の `MermaidBlockWidget` は `mermaid.render` の非同期 SVG を同期プレースホルダへ resolve 後に注入し（`securityLevel: 'strict'` のサニタイズ済み SVG、生 HTML 不挿入）、失敗時は生コードを `<pre>` でフォールバック、`estimatedHeight`＋非同期完了時/`updateDOM` の `requestMeasure` で高さ会計を満たす（R-28-10/11）。編集への移行は `<details>`（R-27-07）と同じく右クリック「Mermaidを編集」からのオプトインのみで、状態は `src/webview/main.ts` の module-level 集合（`mermaidDirectEditStartLines`）が保持し、`pruneMermaidDirectEdit` がキャレット離脱で解除する。Mermaid JS は Webview バンドル同梱、CSP は無変更（ADR-0023）。
