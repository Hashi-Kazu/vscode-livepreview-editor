...
> 🧊 **開発凍結（2026-06-22 時点 / v1.11.0）**
>
> 本プロジェクトは **v1.11.0 をもって開発を凍結**する。
> 凍結理由: **品質安定・スコープ確定**。ライブプレビューの体裁磨き込み（特に `<details>` アコーディオンの編集中表示）はこれ以上の改善が困難と判断し、現状の安定状態（型チェック／build 通過・受け入れテスト 136 件 全 PASS）で打ち止めとする。
> 以降は原則として新規の機能追加・体裁変更を行わない。重大な不具合修正など、やむを得ない場合のみ CLAUDE.md の開発ルール（`feature-dev` 経由）に従って対応する。
>
> 凍結時点の主な内容（v1.10.0〜v1.11.0）:
> - ビューア体裁を「Markdown All in One」プレビューに寄せる磨き込み（左余白拡大、タスクリスト本文をリンク色から本文色へ、本文フォント明示）
> - `<details>` アコーディオンの編集中表示を改善（構造 HTML タグ `<details>/<summary>/</summary>/</details>` を常時非表示化＝R-27-05、本文行のインライン記法描画＝R-28-09、サマリを通常ウェイト＋マーカー余白）
>
> 以前の主な内容（v1.7.0〜v1.9.0）:
...
>
> 凍結を解除して開発を再開する場合は、このバナーを開発継続表記に戻すこと。
