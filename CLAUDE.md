...
> ▶️ **開発継続中（2026-06-22 時点 / v1.12.0）**
>
> v1.11.0 の開発凍結は **v1.12.0 で解除**し、開発を再開した。以降は通常どおり機能追加・変更・バグ修正を行う（すべて CLAUDE.md の開発ルール＝`feature-dev` 経由）。
>
> v1.12.0 の主な内容:
> - 表・`<details>` アコーディオンを**ビューア専用化**（ブロック内カーソルでもウィジェットのまま＝実質非編集、編集は標準ソースエディタ。R-22-02／R-27-03 再定義、R-27-05／R-28-09 を新挙動へ改訂）
> - アコーディオンの開閉状態を summary キーで保持（再描画後も維持。R-27-02）
> - チェックボックス ON/OFF のホスト→Webview 反映を修正（`toggleTask` 後に明示 `update` 送信。R-08-07）
> - 相対パスリンクの山括弧宛先 `[ラベル](<…>)` を `openLink` で除去（R-21-03）
> - 本文余白を `20px 40px 24px 48px` へ調整（R-28-07）
> - ビュー切り替えの保存確認回避を堅牢化（openWith 後にアクティブ以外の重複タブのみ close。R-03-05）
>
> 以前の主な内容（v1.10.0〜v1.11.0）:
> - ビューア体裁を「Markdown All in One」プレビューに寄せる磨き込み（左余白拡大、タスクリスト本文をリンク色から本文色へ、本文フォント明示）
> - `<details>` アコーディオンの編集中表示（v1.12.0 でビューア専用化により再定義）
>
