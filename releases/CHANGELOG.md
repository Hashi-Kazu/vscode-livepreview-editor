...
## v1.21.0 — ビューア操作後の Markdown 切り替えフォーカス復帰 (2026-06-26)

### 追加

- **ビューア操作中に別 Markdown ソースへ切り替えた場合だけ、追従更新後に対象ビューアへフォーカスを戻す挙動を追加（R-03-05/R-03-09）。** `onDidChangeViewState` で直前に active だったビューアを一回限りの復帰状態として記録し、`onDidChangeActiveTextEditor` の `livePreview.followActiveEditor` 追従処理で消費する。対象 URI を既に別ビューアが表示している場合はその既存ビューアへ復帰し、標準エディタ操作中の Markdown 切り替え、非 Markdown、設定 OFF、同一 URI ではフォーカスを奪わない。
