...
- `appendInlineCell`（`src/webview/decorations.ts`）のトークナイズ方式を変更。従来は `<br>` を他のインライン記法（コード/太字/斜体）と同一の正規表現内の1分岐として扱っていたため、セル内に2つの `_` が `<br>` を挟んで存在する場合（例: `report_v1.md<br>final_v2.md`）に `<br>` を跨いでイタリックとして誤マッチし、`<br>` がリテラル文字列のまま表示されることがあった。
- 修正後は、セルテキストを先に `<br\s*\/?>` で分割し、既存トークナイザー（`appendInlineRun` として抽出）をセグメントごとに独立実行、実際の `<br>` 要素で結合する方式に変更。構造的に `<br>` を跨いだ誤マッチが発生しなくなった。
- テスト（`test/feature.issue94.tableCellBrEmphasisBoundary.test.ts`）で境界ケースを検証。
...
- README を現行機能（数式・Mermaid・GitHub Alerts・コードハイライト・見出し折りたたみ・表編集・メディア/URL ペースト・スクロール同期）に合わせて再構成し、package.json の description・keywords を更新。機能コードの変更なし。

## v1.50.0 — 改行編集起因のソース可視範囲変化をプレビューへ中継しない (Issue #74)

## v1.49.0 — 表セル編集中の input で copy/cut/paste を CodeMirror へ誤伝播させない (Issue #71)

## v1.48.0 — ローカル編集起因のスクロールをユーザースクロールとして中継しない (Issue #70)

## v1.47.0 — 表ウィジェットのパディング部分クリックでキャレットが移動しないよう修正 (Issue #55)

## v1.46.0 — 選択がない状態でも単体 URL ペーストをリンク化 (Issue #66)
