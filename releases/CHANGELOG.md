...
## v1.17.2 — ビュー切り替え時の保存ダイアログを抑制・ツールバーアイコン変更 (2026-06-24)

### 修正

- **ビュー切り替え時（Live ↔ Source）に未保存の編集があると保存確認ダイアログが表示されるバグを修正。** `switchEditor` 内で revert → switch → restore パターンを採用した。切り替え前にダーティな内容を退避し、`workbench.action.revertFile` でドキュメントをクリーン状態に戻してからステールタブを閉じ、その後 `workspace.applyEdit` で元の内容を復元する。ダイアログが出なくなり、かつ編集内容は保持される（R-03-05）。

### 変更

- **ツールバーの「Live Preview エディタで開く」ボタンのアイコンをプラグイン自身のアイコン（`media/icon.png`）に変更。**
